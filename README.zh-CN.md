...
无需安装，也没有构建步骤 —— 浏览器直接跑原生 ES module，偶尔用到的库随项目一起放着。`pnpm dev` 跑的是 [scripts/serve.mjs](scripts/serve.mjs)，五十行、只用 Node 自带模块的静态服务（ES module 不能从 `file://` 加载，所以得有个 server）。它发 `no-store`，这条比听起来重要：会回 `304` 的服务器会让浏览器继续用旧的 ES module 或 `registry.json`，最后你调的是一个「一半新一半旧」的页面。
